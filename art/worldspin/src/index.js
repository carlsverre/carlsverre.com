...
for (var i = 0; i < waterG.vertices.length; i++) {
    ...
    v.multiplyScalar(
        1 +
        (-.02 * noise.simplex3(v.x * r, v.y * r, v.z * r))
    );
}
...
for (var i = 0; i < geometry.vertices.length; i++) {
    ...
    v.multiplyScalar(
        1 +
        (.1 * noise.simplex3(v.x * x, v.y * x, v.z * x)) +
        (.05 * noise.simplex3(v.x * y, v.y * y, v.z * y)) +
        (.02 * noise.simplex3(v.x * z, v.y * z, v.z * z))
    );
}
...
var particleField = new THREE.Points(particles, new THREE.PointsMaterial({
    size: 0.01,
    color: 0xeeccff,
}));
...
for (var i = 0; i < 32; ++i) {
    ...
    waveM.myopacity = 0;
    waveM.idx = i;
    waveM.rotation.x = .1 * Math.random();
    waveM.rotation.y = .1 * Math.random();
    waveM.rotation.z = .1 * Math.random();
    waves.push(waveM);
    scene.add(waveM);
}
...
function render() {
    ...
    if (frameI % 1 === 0) {
        ...
        for (var i = 0; i < waves.length; i++) {
            ...
            waves[i].material.opacity = waves[i].myopacity * ((waves.length - i) / waves.length * 1.5);
        }
    }
